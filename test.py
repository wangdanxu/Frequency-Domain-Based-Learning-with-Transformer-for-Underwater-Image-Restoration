...
def test(netG,epoch):
    ...
    savepath = 'netG_' + str(epoch) + '/'
    result_dir = './facades/'+str(SCALE)+savepath
    ...
    with tqdm(total=len(total_files)) as t:
        ...
        for m in total_files:
            ...
            dataset_torchx = dataset_torchx.permute(0, 1, 3, 2)

            output=atmos_net(dataset_torchx)
            output = (output.clamp_(0.0, 1.0)[0].detach().cpu().numpy().transpose(2,1, 0) * 255.0).astype(np.uint8)
            output = output[:, :, ::-1]
