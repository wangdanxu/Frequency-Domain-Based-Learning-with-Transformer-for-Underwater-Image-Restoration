...
class ToTensor(object):
    def __call__(self, sample):
        ...
        hazy_image = hazy_image.permute(0, 2, 1)
        ...
        clean_image = clean_image.permute(0, 2, 1)
        return {'hazy': hazy_image,
                'clean': clean_image}


class Dataset_Load(Dataset):
    def __init__(self, data_path, scale, transform=None):

        self.data_path = data_path
        self.SCALE = scale

        if (self.SCALE == 2):
            self.lr_res_ = (320, 240)
        elif (self.SCALE == 3):
            self.lr_res_ = (214, 160)
        else:
            self.lr_res_ = (160, 120)

        self.lr_files, self.hr_files = self.get_file_paths(self.data_path, 'UFO-120')
        self.len = min(len(self.lr_files), len(self.hr_files))
    ...
    def __getitem__(self, index):
        hazy_im = cv2.resize(cv2.imread(self.lr_files[index % self.len]), self.lr_res_,
                             interpolation=cv2.INTER_CUBIC)
        hazy_im = hazy_im[:, :, ::-1]  ## BGR to RGB
        hazy_im = np.float32(hazy_im) / 255.0

        clean_im = cv2.imread(self.hr_files[index % self.len])
